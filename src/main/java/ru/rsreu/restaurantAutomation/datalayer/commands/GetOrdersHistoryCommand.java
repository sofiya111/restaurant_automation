...
public class GetOrdersHistoryCommand implements Command {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp) {
        try {
            ...
            List<Reservation> reservation = myOrderDAO.getReservationsUser(user.getId());
            session.setAttribute(ORDERS_ATTRIBUTE_NAME,reservation);
            req.getRequestDispatcher(ORDERS_USER_PATH).forward(req, resp);
        } catch (IOException e) {
